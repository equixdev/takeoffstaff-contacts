...
export const Register = () => {
	...
	const handleRegistration = (element) => {
		...
		}).then(res => res.json()).then(isUserNew => {
			isUserNew ? store.dispatch(loginUser(login)) : setCredentialsIncorrect(true)
		})
	}

	return (
			<div className="max-w-xs p-2">
			<h2 className="text-xl font-medium m-2">Регистрация</h2>
				<form onSubmit={handleRegistration}>
					{areCredentialsIncorrect && <p className="m-2 text-red-600">Пользователь с таким логином уже существует</p>}
					...
					<input className="w-72 m-2 bg-blue-600 text-white rounded-lg px-4 py-2" type="submit" value="Зарегистрироваться"/>
				</form>
			</div>
	);
}